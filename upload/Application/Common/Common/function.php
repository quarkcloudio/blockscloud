...
// 定义积木云版本
const BLOCKSCLOUD_VERSION = '1.0.0_dev';
...
function load_theme_function()
{
    if (file_exists('./Application/Common/Conf/config.php')) {
        ...
        $DB              = array();
        $DB['DB_TYPE']   = $config['DB_TYPE'];
        $DB['DB_HOST']   = $config['DB_HOST'];
        $DB['DB_NAME']   = $config['DB_NAME'];
        $DB['DB_USER']   = $config['DB_USER'];
        $DB['DB_PWD']    = $config['DB_PWD'];
        $DB['DB_PORT']   = $config['DB_PORT'];
        $DB['DB_PREFIX'] = $config['DB_PREFIX'];

        $connect = mysqli_connect($DB['DB_HOST'].':'.$DB['DB_PORT'],$DB['DB_USER'],$DB['DB_PWD']);

        //判断是否能链接数据库
        if ($connect) {
            $db     = Db::getInstance($DB);
            $hastable  = $db->query("SHOW TABLES LIKE '".$DB['DB_PREFIX']."theme'");
            //判断是否存在theme主题表
            if ($hastable) {
                $sql    = "select * from ".$DB['DB_PREFIX']."theme where is_setup=1 ";
                $theme  = $db->query($sql);
                if (!empty($theme[0]['name'])) {
                    //存在自定义主题函数，则加载函数并初始化主题名称！
                    C('SITE_THEME',$theme[0]['name']);
                    if (file_exists('./Theme/'.$theme[0]['name'].'/Common/function.php')) {
                        require_once('./Theme/'.$theme[0]['name'].'/Common/function.php');
                    }
                }
            }
            //释放链接
...
function get_avatar($uid = 0){
    ...
    $result = M('Avatar')->where(array('uid'=>$uid,'status'=>1))->getField('path');
    if (empty($result)) {
        $result = './Public/Home/images/default.jpg';
    }
    return $result;
}
 ...
 * @return string       用户字段值
 */
function get_userinfo($uid = 0,$field){
    if (empty($uid)) {
        $uid = is_login();
    }
    $result = M('Member')->where(array('uid'=>$uid,'status'=>1))->getField($field);
    if (empty($result)) {
        $result = '这家伙很懒，一个字都没留下！';
    }
    return $result;
}
...
/**
 * use_theme  使用主题方法
 * @param   $template  数据库中的模板
 * @author  tangtanglove
 */
function use_theme($template)
{
    if (C('SITE_THEME')) {
        //设置主题PUBLIC
        if (!empty($template)) {
            C('TMPL_PARSE_STRING',array('__THEME_STATIC__'  => __ROOT__ . '/Theme/'.C('SITE_THEME').'/Static','__STATIC__' => __ROOT__ . '/Public/static'));
            $template_path = "./Theme/".C('SITE_THEME')."/".MODULE_NAME."/".CONTROLLER_NAME."/".$template.C('TMPL_TEMPLATE_SUFFIX');
            ...
            C('TMPL_PARSE_STRING',array('__THEME_STATIC__'  => __ROOT__ . '/Theme/'.C('SITE_THEME').'/Static','__STATIC__' => __ROOT__ . '/Public/static'));
            $template_path = "./Theme/".C('SITE_THEME')."/".MODULE_NAME."/".CONTROLLER_NAME."/".ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX');
            if (is_file($template_path)) {
                return $template_path;
            }else{
                return $template;
...
function send_sms($phone,$content)
{
    if(preg_match("/^1[34578]\d{9}$/", $phone)){
        //在短信宝注册的账户名和密码
        $sms = new SmsBao(C('SMSBAO_USERNAME'), C('SMSBAO_PASSWORD'));
        $str = $sms->sendSms($phone,$content);//支持批量发送，多个手机号之间用英文逗号分隔，返回值为数组
