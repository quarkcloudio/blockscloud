 ...
 * 引入其他文件函数
 */
require_once(APP_PATH . '/Admin/Common/common.php');
require_once(APP_PATH . '/Admin/Common/file.php');
require_once(APP_PATH . '/Admin/Common/util.php');
require_once(APP_PATH . '/Admin/Common/web.php');

/**
 * 使用桌面模板时必要的常量配置
 */
function desktop_define()
{
    //设置常量
    $web_root = P($_SERVER['DOCUMENT_ROOT']).'/';
    define('WEB_ROOT',$web_root);
    define('WEB_HOST','http://'.$_SERVER['HTTP_HOST']);
    define('APP_HOST','http://'.$_SERVER['HTTP_HOST'].'/'.__ROOT__);
    define('BASIC_PATH',$web_root.__ROOT__);
    define('ZONE_PATH',$web_root.__ROOT__.'/Data/Cloud');
    define('ZONE_USER_PATH',ZONE_PATH.'/user');
    define('PUBLIC_PATH',ZONE_PATH.'/public');
    define('DATA_THUMB',ZONE_PATH.'/thumb/');
    define('USER_SYSTEM',ZONE_PATH.'/system/');
    define('USER_RECYCLE',ZONE_USER_PATH.'/'.get_username(UID).'/recycle/');
    define('USER_HOME',ZONE_USER_PATH.'/'.get_username(UID).'/home/');
    define('USER_DESKTOP',ZONE_USER_PATH.'/'.get_username(UID).'/home/desktop/');
    define('USER_TEMP',ZONE_USER_PATH.'/'.get_username(UID).'/data/temp/');
    define('STATIC_PATH','./Public/static/cloud/');

    //不存在用户桌面则创建
    if (!is_dir(USER_DESKTOP)) {
        $src   = './Application/Install/Data/user';
        $des   = './Data/Cloud/user/'.get_username(UID);
        recurse_copy($src,$des);
    }
}
 ...
 */
function get_setup_status($setup){
    if ($setup) {
        $result = '使用中';
    }else{
        $result = '启用';
 ...
 * @author tangtanglove <dai_hang_love@126.com>
 */
function get_app_img($name,$app){
    switch ($app) {
        case 'addons':
            $img_path = './Addons/'.$name.'/logo.jpg';
            if (is_file($img_path)) {
                $result = $img_path;
            }else{
                $result = './Public/static/cloud/images/app/app.png';
            }
            break;
        case 'app':
            $img_path = './Application/'.$name.'/logo.jpg';
            if (is_file($img_path)) {
                $result = $img_path;
            }else{
                $result = './Public/static/cloud/images/app/app.png';
