...
function show_json($data,$code = true,$info=''){
	...
	header("X-Powered-By: blockscloud.");
 ...
 * @return string 
 */
function cloud_msubstr($str, $start = 0, $length, $charset = "utf-8", $suffix = true){
...
function rand_string($len = 4, $type='check_code'){
	...
	if ($type != 4) {
		...
		// 中文随机字
		for($i = 0; $i < $len; $i ++) {
			$str .= cloud_msubstr($chars, floor(mt_rand(0, mb_strlen($chars, 'utf-8') - 1)), 1);
		} 
	} 
	return $str;
} 
